...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname 2017S-F-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(require spd/tags)
...
(@htdd Cat)
(define-struct cat (name species))
;; Cat is (make-cat String String)
;; interp. a cat with its name and species


(@htdd Dog)
(define-struct dog (name species vicious?))
;; Dog is (make-dog String String Boolean)
;; interp. a dog with its name and species,
;;         and its vicious nature: true if vicious, false otherwise


(@htdd Kennel)
(define-struct kennel (id size cats dogs))
;; Kennel is (make-kennel String Natural (listof Cat) (listof Dog))
;; interp. a kennel with its branch id and size in sq ft
;;         the cats and dogs boarded
...
;; The get-emails function consumes a list of contact and a last
...
;;         last name (ln) and email address
...
(@template (listof Contact))

(define (get-emails loc lnm)
  (cond [(empty? loc) empty]
        [else
         (if (match? (first loc) lnm)
             (cons (contact-email (first loc))
                   (get-emails (rest loc) lnm))
             (get-emails (rest loc) lnm))]))
...
(define (get-emails loc lnm)
  (map contact-email (filter (λ (c) (match? c lnm)) loc)))
...
#;#;
(@template (listof Contact) accumulator)

(define (get-emails loc0 lnm)
  ;; emails is (listof String) ;emails of contacts with lnm seen so far
  (local [(define (get-emails loc emails)
            (cond [(empty? loc) emails]
                  [else
                   (if (match? (first loc) lnm)
                       (get-emails (rest loc)
                                   (append emails
                                           (list (contact-email (first loc)))))
                       (get-emails (rest loc) emails))]))]
    (get-emails loc0 empty)))
...
(define (get-emails loc lnm)
  ;; emails is (listof String) ;emails of contacts with lnm seen so far
  (local [(define emails empty)]
    (begin
      (for-each
       (λ (c)
         (if (match? c lnm)
             (set! emails (append emails (list (contact-email c))))
             (void)))
       loc)
      emails)))
...
(@htdf all-country-airports)
(@signature Airport String -> (listof String))
;; produce list of airport codes in c that are reachable from air0
(check-expect (all-country-airports SFO "Canada") empty)
(check-expect (all-country-airports GLA "Canada") (list "YYZ" "YVR"))
(check-expect (all-country-airports YVR "United Kingdom") (list "LHR" "LPL"))
(check-expect (all-country-airports GLA "United Kingdom")
              (list "GLA" "LPL" "LHR"))

(@template Airport (listof Airport) accumulator encapsulated)

(define (all-country-airports air0 c)
  ;; visited is (listof String) - list of the codes of the airports seen so far
  ;; todo is (listof Airport) - list of airports to visit
  ;; rsf is (listof String) - airport codes in country c seen so far
  (local [(define (fn-for-airport a visited todo rsf)
            (if (member? (airport-code a) visited)
                (fn-for-loa visited todo rsf)
                (if (string=? c (airport-country a))
                    (fn-for-loa (cons (airport-code a) visited)
                                (append (airport-dests a) todo)
                                (append rsf (list (airport-code a))))
                    (fn-for-loa (cons (airport-code a) visited)
                                (append (airport-dests a) todo)
                                rsf))))
          
          (define (fn-for-loa visited todo rsf)
            (cond [(empty? todo) rsf]
                  [else
                   (fn-for-airport (first todo) visited
                                   (rest todo) rsf)]))]
    (fn-for-airport air0 empty empty empty)))
...
(define (sum-squares n)
  (foldr + 0 (map sqr (build-list n add1))))
...
(define (sum-squares n)
  ;; sum is Natural - the sum of squares of numbers seen so far
  (local [(define sum 0)]
    (begin
      (for-each (λ (n)
                  (set! sum (+ (sqr n) sum )))
...
(@htdf fold-paper)
(@signature (String Natural Z -> Y) (Y Z -> Z) Z Paper -> Y)
;; abstract fold for paper
(check-expect (fold-paper make-paper cons empty SOLAR) SOLAR) ; copy
(check-expect (local [(define (c1 ttl cite rflop) (+ cite rflop))]
                (fold-paper c1 + 0 SOLAR))
              (+ 115 4091 89 192 345 55)) ; count citations

(@template Paper (listof Paper) encapsulated)

(define (fold-paper c1 c2 b p)
  (local [(define (fn-for-paper p)   ; Paper -> Y
            (c1 (paper-ttl p)
                (paper-cite p)
                (fn-for-lop (paper-refs p))))
          
          (define (fn-for-lop lop)   ; (listof Paper) -> Z
            (cond [(empty? lop) b]
                  [else
                   (c2 (fn-for-paper (first lop))
                       (fn-for-lop (rest lop)))]))]
    (fn-for-paper p)))
...
(@template Flow River use-abstract-fn encapsulated)
...
(define (count-rivers r)
  (local[(define (fn-for-flow f)
         ...
         (define (fn-for-lor lor)
           (foldr (λ (r ror) (+ (fn-for-river r) ror))
                  0 lor))]
    
    (fn-for-river r)))


(@template Flow River use-abstract-fn encapsulated) ; fn-composition if
                                                    ; foldr map solution used
...
(define (flow-through r st-nm)
  (local[(define (fn-for-flow f)
         ...
         (define (fn-for-lor lor) ; foldr map solution
           (foldr append empty (map fn-for-river lor)))
         
         #;
         (define (fn-for-lor lor) ; foldr solution
           (foldr (λ (r ror) (append (fn-for-river r) ror))
                  empty lor))
         
         #;
         (define (fn-for-lost lost)
           (local [(define (match? st)
                     (string=? st-nm st))]
             (ormap match? lost)))
